...
pub async fn exec_add(project_dir: &Path, packages: &[String], dev: bool) -> PymgrResult<()> {
    ...
    for pkg_spec in packages {
        ...
        let wheel = pypi::find_best_wheel(&info.urls).ok_or_else(|| {
            PymgrError::coded(
                ...
                format!("No compatible wheel for {}", name),
            )
        })?;

        downloader::download_single(&name, &resolved_version, &site_packages).await?;
...
pub async fn exec_install(project_dir: &Path, frozen: bool) -> PymgrResult<()> {
    ...
    if lockfile_path.exists() {
        ...
        let site_packages = get_site_packages(project_dir, &python_version)?;
        downloader::download_packages(&lockfile.packages, &site_packages).await?;
        ...
        let site_packages = get_site_packages(project_dir, &python_version)?;
        downloader::download_packages(&resolved, &site_packages).await?;
