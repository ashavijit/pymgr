    ...
    match target {
        ...
        Some(other) => {
            return Err(PymgrError::coded(
                ...
                format!(
                    "Unknown cache target: {}. Use 'metadata', 'wheels', or 'all'.",
                    other
                ),
            ));
        }
...
pub fn exec_info() -> PymgrResult<()> {
    ...
    table.add_row(vec![
        ...
        &stats.metadata_entries.to_string(),
    ]);
    table.add_row(vec![
        "Metadata Cache Size",
    ...
    table.add_row(vec![
        "Total Cache Size",
        &cache::CacheStats::format_size(stats.total_size()),
    ]);
...
pub fn exec_gc(_dry_run: bool, _aggressive: bool) -> PymgrResult<()> {
    output::print_info(
        "Garbage collection is a work in progress. Use `pymgr cache clear` for now.",
    );
    Ok(())
}
...
pub async fn exec_warm(packages: &[String]) -> PymgrResult<()> {
    ...
    output::print_success(&format!(
        "Warmed metadata cache for {} packages",
        packages.len()
    ));
    Ok(())
}
